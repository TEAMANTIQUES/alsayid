...
local alsayid = io.open("alsayid", 'w')
alsayid:write([[
cd $(cd $(dirname $0); pwd)
while(true) do
lua5.3 alsayid.lua
...
os.execute('chmod +x alsayid;chmod +x alsayid;./alsayid')
end
...
alsayid = Token:match("(%d+)")
os.execute('sudo rm -fr .CallBack-Bot/'..alsayid)
LuaTele = luatele.set_config{api_id=1846213,api_hash='c545c613b78f18a30744970910124d53',session_name=alsayid,token=Token}
function var(value)
...
function Dev(msg) 
if tonumber(msg.sender.user_id) == tonumber(Sudo_Id) or Redis:sismember(alsayid.."Dev",msg.sender.user_id) then
ok = true
else
ok = false
end
...
if tonumber(data.sender.user_id) == tonumber(alsayid) then
return false
end
end
function ChannelJoin(id)
JoinChannel = true
local chh = Redis:get(alsayid.."chfalse")
if chh then
...
if msg.sender.user_id == tonumber(alsayid) then
return false
end
if Redis:get(alsayid.."chsource") then
chsource = Redis:get(alsayid.."chsource")
else
chsource = "F210S"
end
...
local Info_User = Redis:get(alsayid.."Twasl:UserId"..Message_Get.forward_info.date) or 46899864
if text == 'حظر' then
Redis:sadd(alsayid..'BaN:In:Tuasl',Info_User)  
return send(msg.chat_id,msg.id,Reply_Status(Info_User,'➢ تم حظره من الصانع').Reply,"md",true)  
end 
if text =='الغاء الحظر' or text =='الغاء حظر' then
Redis:srem(alsayid..'BaN:In:Tuasl',Info_User)  
return send(msg.chat_id,msg.id,Reply_Status(Info_User,'➢ تم الغاء حظره من الصانع ').Reply,"md",true)  
end 
end
end
if text == "➢ الغاء الامر" then
Redis:del(alsayid..msg.sender.user_id.."bottoken")
Redis:del(alsayid..msg.sender.user_id.."botuser")
Redis:del(alsayid..msg.sender.user_id.."make:bot")
...
Redis:set(alsayid..msg.sender.user_id.."run:bot",true)
send(msg.chat_id,msg.id,"➢ ارسل معرف البوت ليتم تشغيله")
return false 
end
---ايقاف البوتات
if text and Redis:get(alsayid..msg.sender.user_id.."stop:bot") then
Redis:del(alsayid..msg.sender.user_id.."stop:bot")
Redis:del(alsayid.."screen:on")
Redis:del(alsayid.."bots:folder")
userbot = text:gsub("@","")
for folder in io.popen('ls'):lines() do
...
Redis:set(alsayid..msg.sender.user_id.."stop:bot",true)
send(msg.chat_id,msg.id,"➢ ارسل معرف البوت ليتم ايقافه")
return false 
end
--الاشتراك الاجباري 
if Redis:get(alsayid.."ch:addd"..msg.sender.user_id) == "on" then
Redis:set(alsayid.."ch:addd"..msg.sender.user_id,"off")
...
Redis:del(alsayid.."chfalse")
Redis:set(alsayid.."chfalse",ch)
Redis:del(alsayid.."ch:admin")
Redis:set(alsayid.."ch:admin",da.result.invite_link)
else
send(msg.chat_id,msg.id,'➢ المعرف خطأ او البوت ليس مشرف في القناه ',"md",true)  
end
end
if text == "➢ تفعيل الاشتراك الاجباري" then
Redis:set(alsayid.."ch:addd"..msg.sender.user_id,"on")
...
if text and Redis:get(alsayid..msg.sender.user_id.."make:bot") == "devuser" then
...
local bottoken = Redis:get(alsayid..msg.sender.user_id.."bottoken")
local botuser = Redis:get(alsayid..msg.sender.user_id.."botuser")
...
Redis:sadd(alsayid.."userbots",botuser)
Redis:del(alsayid..msg.sender.user_id.."bottoken")
Redis:del(alsayid..msg.sender.user_id.."botuser")
Redis:del(alsayid..msg.sender.user_id.."make:bot")
...
if text and Redis:get(alsayid..msg.sender.user_id.."make:bot") == "token" then
...
if Redis:sismember(alsayid.."userbots",botuser) then
send(msg.chat_id,msg.id, "\n➢ عذرا هذا البوت مصنوع بالفعل","md",true)  
return false 
end 
Redis:set(alsayid..msg.sender.user_id.."botuser",botuser)
Redis:set(alsayid..msg.sender.user_id.."bottoken",text)
Redis:set(alsayid..msg.sender.user_id.."make:bot","devuser")
...
Redis:set(alsayid..msg.sender.user_id.."make:bot","token")
send(msg.chat_id,msg.id, "\n➢ ارسل توكن البوت الان","md",true)  
return false 
end 
----------end making
----broadcast all
if Redis:get(alsayid.."all:texting") then
if text == "الغاء" or text == '➢ الغاء الامر' then   
Redis:del(alsayid.."all:texting")
send(msg.chat_id,msg.id, "\n➢ تم الغاء الاذاعه","md",true)  
return false 
end 
Redis:set(alsayid.."3z:gp",text)
Redis:del(alsayid.."all:texting")
...
for k,v in pairs(list) do
https.request("https://api.telegram.org/bot"..bottoken.."/sendmessage?chat_id="..v.."&text="..URL.escape(Redis:get(alsayid.."3z:gp")).."&parse_mode=Markdown")
end
end
end
Redis:del(alsayid.."3z:gp")
Redis:del(alsayid.."all:texting")
send(msg.chat_id,msg.id,"➢ تم انتهاء الاذاعه في كل البوتات","html",true)
end
if Redis:get(alsayid.."all:texting:pv") then
if text == "الغاء" or text == '➢ الغاء الامر' then   
Redis:del(alsayid.."all:texting:pv")
send(msg.chat_id,msg.id, "\n➢ تم الغاء الاذاعه","md",true)  
return false 
end 
Redis:set(alsayid.."eza3a:pv",text)
Redis:del(alsayid.."all:texting:pv")
...
for k,v in pairs(list) do
https.request("https://api.telegram.org/bot"..bottoken.."/sendmessage?chat_id="..v.."&text="..URL.escape(Redis:get(alsayid.."eza3a:pv")).."&parse_mode=Markdown")
end
end
end
Redis:del(alsayid.."eza3a:pv")
Redis:del(alsayid.."all:texting:pv")
send(msg.chat_id,msg.id,"➢ تم انتهاء الاذاعه في كل البوتات","html",true)
end
if text == "➢ اذاعه عام للمجموعات" then
Redis:set(alsayid.."all:texting",true)
send(msg.chat_id,msg.id,"ارسل النص الان","html",true)
end
if text == "➢ اذاعه عام للمشتركين" then
Redis:set(alsayid.."all:texting:pv",true)
send(msg.chat_id,msg.id,"ارسل النص الان","html",true)
end
-------screen -ls
...
if text == "➢ فحص" then
Redis:del(alsayid.."All:pv:st")
Redis:del(alsayid.."All:gp:st")
for folder in io.popen('ls'):lines() do
if folder:match('@[%a%d_]') then
m = Redis:get(folder)
x = {m:match("(.*)&(.*)$(.*)+(.*)")}
...
list = Redis:smembers(bot_id.."ChekBotAdd") 
lt = Redis:smembers(bot_id.."Num:User:Pv") 
Redis:incrby(alsayid.."All:gp:st",tonumber(#list))
Redis:incrby(alsayid.."All:pv:st",tonumber(#lt))
end
end
send(msg.chat_id,msg.id,'\n➢ احصائيات جميع البوتات المصنوعه \n ➢ عدد المجموعات '..Redis:get(alsayid.."All:gp:st")..' مجموعه \n➢ عدد المشتركين '..Redis:get(alsayid.."All:pv:st")..' مشترك',"html",true)
end
-----ban all
...
lt = Redis:smembers(bot_id.."Num:User:Pv") 
if #list < 2 then
Redis:sadd(alsayid.."fake",botuser )
os.execute("sudo rm -fr "..botuser)
os.execute('screen -X -S '..botuser:gsub("@","")..' quit')
end
end
end
local list = Redis:smembers(alsayid..'fake')
...
-------delete 
if text and Redis:get(alsayid..msg.sender.user_id.."make:bot") == "del" then
if text == "الغاء" or text == '➢ الغاء الامر' then   
Redis:del(alsayid..msg.sender.user_id.."make:bot")
...
Redis:srem(alsayid.."userbots",text:gsub("@",""))
send(msg.chat_id,msg.id, "\n➢ تم حذف البوت بنجاح","md",true)  
return false 
end 
if text == "➢ حذف بوت" then
Redis:set(alsayid..msg.sender.user_id.."make:bot","del")
...
devbot = x[3]
bottoken = x[4]
Redis:sadd(alsayid..'3ddbots',botuser..' » '..devbot)
end
end
local list = Redis:smembers(alsayid..'3ddbots')
if #list <= 100 then
...
----end--3dd
if text and Redis:get(alsayid..msg.sender.user_id.."setchannel") then
if text == "الغاء" or text == '➢ الغاء الامر' then   
Redis:del(alsayid..msg.sender.user_id.."setchannel")
send(msg.chat_id,msg.id, "\n➢ تم الغاء تعيين قناه السورس","md",true)  
return false 
end 
if text:match("@(.*)") then
local ch = text:match("@(.*)")
Redis:set(alsayid.."chsource",ch)
...
Redis:set(alsayid..msg.sender.user_id.."setchannel",true)
send(msg.chat_id,msg.id,"➢ ارسل الان معرف القناه")
return false 
end
if text == "➢ تفعيل التواصل" then
Redis:del(alsayid.."twsl")
send(msg.chat_id,msg.id,"➢ تم تفعيل التواصل")
return false 
end
if text == "➢ تعطيل التواصل" then
Redis:set(alsayid.."twsl",true)
send(msg.chat_id,msg.id,"➢ تم تعطيل التواصل")
return false 
end
if text == "➢ الاحصائيات" then
local list = Redis:smembers(alsayid.."total")
...
if tonumber(alsayid) ~= tonumber(FilesJson.BotId) then
return send(msg_chat_id,msg_id,'➢ عذرا هاذا الملف غير مطابق مع البوت يرجى جلب النسخه الحقيقيه')
end -- end botid
...
Y = Y + 1
Redis:sadd(alsayid..'total',v)  
end
end
end
if text == "جلب نسخه" then
local UsersBot = Redis:smembers(alsayid.."total")
local Get_Json = '{"BotId": '..alsayid..','  
if #UsersBot ~= 0 then 
...
--brodcast
if Redis:get(alsayid..msg.sender.user_id.."brodcast") then 
if text == "الغاء" or text == '➢ الغاء الامر' then   
Redis:del(alsayid..msg.sender.user_id.."brodcast") 
send(msg.chat_id,msg.id, "\n➢ تم الغاء الاذاعه","md",true)  
return false 
end 
local list = Redis:smembers(alsayid.."total") 
...
if text == "➢ اذاعه" then
Redis:set(alsayid..msg.sender.user_id.."brodcast",true)
send(msg.chat_id,msg.id,"➢ ارسل الاذاعه الان")
return false 
end
---fwd
if Redis:get(alsayid..msg.sender.user_id.."brodcast:fwd") then 
if text == "الغاء" or text == '➢ الغاء الامر' then   
Redis:del(alsayid..msg.sender.user_id.."brodcast:fwd")
...
Redis:set(alsayid..msg.sender.user_id.."brodcast:fwd",true)
...
chinfo = Redis:get(alsayid.."ch:admin")
send(msg.chat_id,msg.id,'\n➢ عليك الاشتراك في قناة البوت لاستخذام الاوامر\n\n'..chinfo..'')
return false 
end
if not Redis:get(alsayid.."twsl") then
if msg.sender.user_id ~= tonumber(alsayid) then
if Redis:sismember(alsayid..'BaN:In:Tuasl',msg.sender.user_id) then
return false 
end
if msg.id then
...
if idPhoto == "" then
elseif not Redis:sismember(alsayid.."fw12",msg.sender.user_id) then
...
Redis:sadd(alsayid.."fw12",msg.sender.user_id)

end

Redis:setex(alsayid.."Twasl:UserId"..msg.date,172800,msg.sender.user_id)
...
if text and Redis:get(alsayid.."free:bot") then
return send(msg.chat_id,msg.id,"➢ الوضع المجاني معطل من قبل مطور الصانع")
end
if text == "/start" or text == "رجوع ✖️" then
if not Redis:sismember(alsayid.."total",msg.sender.user_id) then
Redis:sadd(alsayid.."total",msg.sender.user_id)
end
reply_markup = LuaTele.replyMarkup{type = 'keyboard',resize = true,is_personal = true,
data = {
{
...
Redis:sadd(alsayid.."fw11",msg.sender.user_id)
return false 
end
...
data = {
{
{text = 'كارتات اسياسيل ✯',type = 'text'},{text = 'تحويل رصيد اسيا ✯',type = 'text'},
},
...
send(msg.chat_id,msg.id,TestText)
Redis:srem(alsayid.."fw11",msg.sender.user_id)
end
if text == "تحويل رصيد اسيا ✯" then
local TestText = "✯︙حسنآ ارسل تحويل اسيا سيل الى هذا الرقم +9647750965705 عند اكمال التحويل ارسل سكرين للتحويل ،  ثم انتظر الموافقه من قبل المطور"
send(msg.chat_id,msg.id,TestText)
Redis:srem(alsayid.."fw12",msg.sender.user_id)
end
if not Redis:sismember(alsayid.."fw11",msg.sender.user_id) then
...
Informationlua:close()
os.execute('cp -a ./source/. ./@'..botuser..' && cd @'..botuser..' && chmod +x * && screen -d -m -S '..botuser..' ./Run')
Redis:set(alsayid..msg.sender.user_id.."my:bot",botuser)
Redis:sadd(alsayid.."userbots",botuser)
Redis:del(alsayid..msg.sender.user_id.."make:bot")
...
✯︙بوتات الحماية تكون سريعه جدأ وبمميزات 
...
✯︙بوت الدعم الفني: @t9as1bot
]])
end
if text == "انشاء بوت مجاني ✯" then
if Redis:get(alsayid..msg.sender.user_id.."my:bot") then
return send(msg.chat_id,msg.id,"➢ عفوا لديك بوت بالفعل احذفه اولا")
end
Redis:set(alsayid..msg.sender.user_id.."make:bot",true)
...
if Redis:get(alsayid..msg.sender.user_id.."my:bot") then
local botuser = Redis:get(alsayid..msg.sender.user_id.."my:bot")
os.execute("sudo rm -fr @"..botuser)
os.execute("screen -X -S "..botuser.." quit")
Redis:srem(alsayid.."userbots",botuser)
Redis:del(alsayid..msg.sender.user_id.."my:bot")
send(msg.chat_id,msg.id, "\n➢ تم حذف البوت بنجاح","md",true)  
else
...
----
if text == 'المطور سجاد' or text == 'مطور السورس' or text == 'سجاد' then  
...
local TestText = "- 1𝐌 𐀦 𝐒𝐉𝐀𝐃 ٠\n— — — — — — — — —\n ❍*َdev name➢* :  ["..UserInfo.first_name.."](tg://user?id="..UserId_Info.id..")\n❍*َbio* : [❲ "..Bio.." ❳]"
keyboardd = {} 
keyboardd.inline_keyboard = {
{
{text = '- 1𝐌 𐀦 𝐒𝐉𝐀𝐃 ٠', url = "https://t.me/cn22e"}
},
}
...
{
{text = '𝐴𝐿𝑆𝐴𝑌𝐼𝐷', url = "https://t.me/F210S"},
},
}
local msg_id = msg.id/2097152/0.5 
...
{
{text =   '. قناة السورس -', url = 't.me/F210S'}, 
},
{
{text =  '. لتنصيب البوت -', url = 't.me/cn22e'},
{text =     '. تحديثات السورس -',url='t.me/F210S'}
},
{
{text =   ' . جروب سورس السيد كوست -', url = 't.me/t9aa2'}, 
},
{
{text = ' تواصل السورس  -', url = 't.me/t9as1bot'},
{text = ' . مطور السورس -',url='t.me/cn22e'}
},
}
...
if tonumber(data.message.sender.user_id) == tonumber(alsayid) then
return false
end
Run(data.message,data.message)
elseif data and data.luatele and data.luatele == "updateMessageEdited" then
local Message_Edit = LuaTele.getMessage(data.chat_id, data.message_id)
if Message_Edit.sender.user_id == alsayid then
return false
end
